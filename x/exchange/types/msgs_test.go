...
func TestMsgCreateMarket(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			func(msg *types.MsgCreateMarket) {
				msg.BaseDenom = "uollo"
				msg.QuoteDenom = "uollo"
			},
			"base denom and quote denom must not be same: uollo: invalid request",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			senderAddr := utils.TestAddress(1)
			msg := types.NewMsgCreateMarket(senderAddr, "uollo", "uusd")
...
func TestMsgSwapExactAmountIn(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			"negative input",
			func(msg *types.MsgSwapExactAmountIn) {
				msg.Input = sdk.DecCoin{Denom: "uollo", Amount: sdk.NewDec(-1000000)}
			},
