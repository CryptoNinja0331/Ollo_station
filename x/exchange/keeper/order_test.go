...
func (s *KeeperTestSuite) TestPlaceLimitOrder() {
	market := s.CreateMarket("uollo", "uusd")
	...
	s.Require().Equal(sdk.NewInt64DecCoin("uollo", 0), resp.Received)
	s.CheckEvent(&types.EventPlaceLimitOrder{}, map[string][]byte{
		...
		"received":          []byte(`{"denom":"uollo","amount":"0.000000000000000000"}`),
	})
	...
	s.Require().Equal(sdk.NewInt64DecCoin("uollo", 5_000000), resp.Paid)
	// Matched at 5.1
	s.Require().Equal(sdk.NewInt64DecCoin("uusd", 25_423500), resp.Received)
	s.CheckEvent(&types.EventPlaceLimitOrder{}, map[string][]byte{
		"executed_quantity": []byte(`"5000000.000000000000000000"`),
		"paid":              []byte(`{"denom":"uollo","amount":"5000000.000000000000000000"}`),
