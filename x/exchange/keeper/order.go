...
func (k Keeper) placeLimitOrder(
	...
	if isBatch || openQty.GTE(utils.OneDec) {
		...
		order = types.NewOrder(
			...
			ctx.BlockHeight(), openQty, sdk.NewDecFromInt(depositCoin.Amount), deadline)
...
func (k Keeper) PlaceMarketOrder(
	...
	if isBuy {
		quote := sdk.NewDecFromInt(spendable.AmountOf(market.QuoteDenom))
		quoteLimit = &quote
		priceLimit = maxPrice
	} else {
		base := sdk.NewDecFromInt(spendable.AmountOf(market.BaseDenom))
		if qty.GT(base) {
			err = sdkerrors.Wrapf(
