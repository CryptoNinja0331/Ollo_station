...
func (s *KeeperTestSuite) TestAtomicSwapInsufficientLiquidity() {
	ordererAddr := s.FundedAccount(2, enoughCoins)
	destinationAddr := "some_mock_destination_address"
	seollot := "some_mock_seollot"
	seollotHash := "some_mock_hash"

	cacheCtx, _ := s.Ctx.CacheContext()

	_, _, err := s.keeper.ExecuteAtomicSwap(
		cacheCtx, ordererAddr, destinationAddr,
		sdk.NewCoins(sdk.NewCoin("uatom", sdk.NewInt(1000))),
		seollot, seollotHash, 1000)
	s.Require().ErrorIs(err, types.ErrSwapNotEnoughLiquidity)
}

func (s *KeeperTestSuite) TestAtomicSwapExactAmountIn_MaxOrderPriceRatio() {
	ordererAddr := s.FundedAccount(2, enoughCoins)
	destinationAddr := "some_mock_destination_address"
	seollot := "some_mock_seollot"
	seollotHash := "some_mock_hash"
...
func (s *KeeperTestSuite) TestSwapInsufficientLiquidity() {
	market1 := s.CreateMarket("uollo", "uusd")
	market2 := s.CreateMarket("uatom", "uollo")
...
func (s *KeeperTestSuite) TestSwapExactAmountIn_MaxOrderPriceRatio() {
	...
	creUsdMarket := s.CreateMarket("uollo", "uusd")
	creFooMarket := s.CreateMarket("uollo", "ufoo")
